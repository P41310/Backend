    ...
    try{
        ...
        if($result){
            ...
            if($maxFecha!=NULL){#se tiene ya creado un horario
                ...
                $sql= "INSERT INTO horario2(fecha,horaInicio,horaFin) VALUES ('$fechaActual','$horaInicio','$horaFin')";
                $query= mysqli_query($conexion,$sql);
                // Obtener el ID del último registro insertado
                $idHorario = mysqli_insert_id($conexion);
                //mysqli_close($conexion);
        ...
        */
        $sql="INSERT INTO backend(fecha,turno,empleado,hora,comcode,modelo,cantidad,ProcRe,MatPara,idHorario,estado) VALUES('$fecha','$turno','$empleado','$hora','$comcode','$modelo','$cantidad','$ProcRe', '$MatPara', $idHorario,0)";
